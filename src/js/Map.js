...
class Map{
    ...
    /*    this.map.on('click', function(e){
            ...
            fetch(`http://api.geonames.org/countryCodeJSON?lat=${e.latlng.lat}&lng=${e.latlng.lng}&username=stevany`)
           // fetch(`'http://api.geonames.org/countryCodeJSON?lat='+e.latlng.lat+'&lng='+e.latlng.lng+'&username=stevany'`)
            
          //  http://maps.googleapis.com/maps/api/geocode/json?latlng='+e.latlng.lat+','+e.latlng.lng+'&sensor=true'`)
            .then((data) =>{
            ...
            let zip = data.results[0].address_components[7].long_name;
...
/*
            ...
            //    console.log(circleRadius);
                if (element.CountryCode === country.alpha2Code ) {
                 ...
                 //   console.log('circle');
                   ...
                   //    const lng = coord.lng;
                    //   console.log("You clicked the map at latitude: " + lat + " and longitude: " + lng);
                        table.changeTableViaInput(e.target.options.country);
                        dom.checkBox.checked = true;
                   }); // circle.on
                } // if (element.CountryCode === country.alpha2Code )
            }); // result.resultCOVID.Countries.forEach(element => {
    ...
    changeMap(e){
        ...
        this.map.removeLayer(this.group);
        console.log(e.target.getAttribute('id'));
        this.getData(e.target.getAttribute('id'));
        // this.getData(e.target.innerHTML);
    }
